...
export interface MultiTurnCase {
  ...
  expected: {
    intent: string;
    paramsContain?: Record<string, unknown>;
  };
}
...
export const MULTI_TURN_CASES: MultiTurnCase[] = [
  ...
  { id: "MT-06", description: "'mostra mais' após read_emails", history: [{ role: "user", content: "lê os 3 primeiros emails" }, { role: "assistant", content: "Tens 3 emails de João, Maria e Ana" }], transcript: "mostra mais", expected: { intent: "read_emails" } },
  ...
  { id: "MT-08", description: "refinamento João", history: [{ role: "user", content: "procura o João" }, { role: "assistant", content: "Encontrei 3 Joãos: João Silva, João Costa, João Pereira" }], transcript: "o João Silva", expected: { intent: "contacts_search", paramsContain: { query: "João Silva" } } },
  { id: "MT-09", description: "meio da manhã", history: [], transcript: "o que tenho a meio da manhã", expected: { intent: "calendar_list" } },
  { id: "MT-10", description: "amanhã follow-up", history: [{ role: "user", content: "o que tenho hoje?" }, { role: "assistant", content: "Reunião Maria 15h" }], transcript: "e amanhã?", expected: { intent: "calendar_list" } },
  { id: "MT-11", description: "apaga o último ambíguo", history: [{ role: "assistant", content: "3 emails: João, Maria, Ana" }], transcript: "apaga o último", expected: { intent: "general" } },
  { id: "MT-12", description: "sim confirma", history: [{ role: "assistant", content: "Criar reunião Maria quinta 15h-16h. Confirmas?" }], transcript: "sim confirma", expected: { intent: "general" } },
  ...
  { id: "MT-14", description: "lê o primeiro", history: [{ role: "user", content: "procura emails do João" }, { role: "assistant", content: "2 emails de João Silva" }], transcript: "lê o primeiro", expected: { intent: "read_emails" } },
  { id: "MT-15", description: "olá", history: [], transcript: "olá Vox", expected: { intent: "general" } },
  { id: "MT-16", description: "quanto tempo pergunta", history: [{ role: "assistant", content: "Reunião Maria às 15h" }], transcript: "quanto tempo até à reunião?", expected: { intent: "general" } },
  { id: "MT-17", description: "manda agora", history: [{ role: "assistant", content: "Draft: Olá João, confirmo" }], transcript: "manda agora", expected: { intent: "send" } },
  { id: "MT-18", description: "próxima semana", history: [], transcript: "o que tenho na próxima semana", expected: { intent: "calendar_list" } },
  { id: "MT-19", description: "cria outro às 17h", history: [{ role: "user", content: "marca reunião quinta 15h" }, { role: "assistant", content: "Evento criado" }], transcript: "cria outro às 17h", expected: { intent: "calendar_create" } },
